...
Session.prototype = {
  ...
  joins: function joins(client){
    ...
    client.on('message', function(data){
      if (EVENT_TOKEN === data[0]) {
        var parsed = JSON.parse(data.slice(1));
        if (parsed.cmd === TRACE_EVENT)
    ...
    setTimeout(function resync(){
      if (i < history.length) {
        self.sendRaw(client, history[i++]);
        setTimeout(resync, 1);
      }
    }, 1000);
  },
  ...
  sendRaw: function sendRaw(client, data){
    client.send(data);
  },
